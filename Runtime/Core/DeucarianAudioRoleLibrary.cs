...
{
    /// <summary>
    /// Designer-maintained collection of available audio role assets.
    /// </summary>
    [CreateAssetMenu(fileName = "Audio Role Library", menuName = "Deucarian/Theming/Audio Role Library")]
    public sealed class DeucarianAudioRoleLibrary : ScriptableObject
    {
        [SerializeField] private List<DeucarianAudioRole> roles = new List<DeucarianAudioRole>();

        [NonSerialized] private Dictionary<string, DeucarianAudioRole> roleById;

        /// <summary>Ordered role assets in this library.</summary>
        public IReadOnlyList<DeucarianAudioRole> Roles => roles;

        /// <summary>Finds the first role with the requested stable ID.</summary>
        public bool TryGetRoleById(string id, out DeucarianAudioRole role)
        {
            EnsureCache();

            string normalizedId = DeucarianAudioRole.NormalizeId(id);
        ...
        public bool AddRole(DeucarianAudioRole role)
        {
            EnsureRoleList();
        ...
        {
            EnsureRoleList();
            roleById = new Dictionary<string, DeucarianAudioRole>(StringComparer.Ordinal);
        ...
        private static int CompareRoles(DeucarianAudioRole left, DeucarianAudioRole right)
        {
            if (left == right)
            {
                return 0;
        ...
        private void EnsureRoleList()
        {
            if (roles == null)
            {
                roles = new List<DeucarianAudioRole>();
            }
        }
